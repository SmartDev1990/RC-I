...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({
  ...
  const handleDeposit = async (lockDuration = 604800) => {
    const receipt = await fetchWithCatchTxError(() => {
      // .toString() being called to fix a BigNumber error in prod
      // as suggested here https://github.com/ChainSafe/web3.js/issues/2077
      ...
      // 	const address = '0xf0B02B3Eeb1Fe1057691f4267f62CfCeaa8A81D8'
      // 	const abi = ['function deposit(uint256 _amount, uint256 _lockDuration)']
      // 	const Depocontract = new ethers.Contract(address, abi, library.getSigner())
      // 	console.log(lockDuration.toString(), convertedStakeAmount.toString())
      // 	const tx = await Depocontract.deposit(convertedStakeAmount.toString(), lockDuration.toString(), {
      // 		gasLimit: '3000000',
      // 	})
      // 	console.log('=>>>>>>>>>receipt')
